...
- [ ] 建立 `make fmax_sweep` 自动化目标。
- [ ] 建立每轮优化结果表，记录 period、WNS、TNS、area、critical path、测试结果。

### 5.2 P1: MDU 优化

- [ ] 拆分 `src/mul_div.v` 为 `mul_pipe`、`div_pipe`、`mdu_ctrl` 或等效结构。
- [ ] 乘法路径移除最后一轮 `mul_next` 与符号修正在同一拍完成的问题。
- [ ] 为乘法结果增加 `mul_result_reg`。
- [ ] 为除法最后一轮商/余数符号修正增加 finish stage。
- [ ] 保持 RV32M 边界语义：除零、`0x80000000 / -1`、有符号高位乘法。
- [ ] 更新 `pipeline_cpu_core` 中 MDU stall/done 接口。
- [ ] 跑 `make -C sim/pipeline_test_instr m_type`。
- [ ] 跑 `baseline_a`、`all_tests`、`hazard_forward`。
- [ ] 跑 `make -C syn synth CLK_PERIOD_NS=3.0`。
- [ ] 若 3.0 ns clean，继续 sweep 2.8/2.6/2.4 ns。
...
1. 增加 Fmax sweep 自动化，保证每次优化都有可比较报告。
2. 重构 MDU，把乘法最后一轮加法和符号修正拆开，或者直接切换到 pipeline multiplier。

完成后以 3.0 ns 为第一个新目标周期进行综合闭环。
